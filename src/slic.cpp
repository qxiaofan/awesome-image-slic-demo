 ...
 * Output: -
 */
void Slic::display_contours(IplImage *image, CvScalar colour, std::vector<Point>& contours ) {
...
void Slic::getClusterPoints(std::vector<std::vector<cv::Point > > &spPoints) {
    
    spPoints.resize(centers.size());
    for (int i = 0, width = clusters.size(); i < width; ++i) { 
        for (int j = 0, height = clusters[i].size(); j < height; ++j) {
            int label = clusters[i][j];
            if (-1 != label) {
                spPoints[label].push_back(Point(i,j));
            }
        }
    }
}
